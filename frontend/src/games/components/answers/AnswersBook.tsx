...
function AnswersBook({
  word, data, handleState, state,
}: IProps) {
  const [answers, setAnswers] = useState<IUserWord[] | undefined>();
  const dispatch = useAppDispatch();

  const {
    _id: wordId, wordTranslate, word: wordName,
  } = word;
  ...
  useEffect(() => {
    if (state === 'question') {
      ...
      setAnswers(
        getAnswers({ data, wordId }) as IUserWord[],
      );
    }
  }, [data, state, wordId]);
